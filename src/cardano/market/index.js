...
const CONTRACT_ADDRESS = () =>
  Loader.Cardano.Address.from_bech32(
    "addr1wyr2w40y54euy76g7q0sx3f7xypa4tcg34ny4dhl65zj03gwmdadj"
  );
...
const IDMAKER = () => {
  const datum = Loader.Cardano.PlutusData.new_constr_plutus_data(
    Loader.Cardano.ConstrPlutusData.new(
      Loader.Cardano.Int.new_i32(DATUM_TYPE.IdMaker),
...
const PROPOSAL = ({ proposalStart, scriptUpdate, scriptAddr, spend, spendValue, spendAddr, minting, minted, yes, no }) => {
  const fieldsInner = Loader.Cardano.PlutusList.new();
  fieldsInner.add(Loader.Cardano.PlutusData.new_bytes(fromHex(proposalStart)));
  fieldsInner.add(Loader.Cardano.PlutusData.new_bytes(fromHex(scriptUpdate)));
  fieldsInner.add(Loader.Cardano.PlutusData.new_bytes(fromHex(scriptAddr)));
  fieldsInner.add(Loader.Cardano.PlutusData.new_bytes(fromHex(spend)));
  fieldsInner.add(Loader.Cardano.PlutusData.new_bytes(fromHex(spendValue)));
  fieldsInner.add(Loader.Cardano.PlutusData.new_bytes(fromHex(spendAddr)));
  fieldsInner.add(Loader.Cardano.PlutusData.new_bytes(fromHex(minting)));
  fieldsInner.add(Loader.Cardano.PlutusData.new_bytes(fromHex(minted)));
  fieldsInner.add(Loader.Cardano.PlutusData.new_bytes(fromHex(yes)));
  fieldsInner.add(Loader.Cardano.PlutusData.new_bytes(fromHex(no)));
  const proposal = Loader.Cardano.PlutusList.new();
  proposal.add(
  ...
  const datum = Loader.Cardano.PlutusData.new_constr_plutus_data(
    Loader.Cardano.ConstrPlutusData.new(
      Loader.Cardano.Int.new_i32(DATUM_TYPE.Proposal),
      proposal
    )
  );
  return datum
...
const OWNERSHIP = ({ owner, firstSlot, lastTransfer }) => {
  const fieldsInner = Loader.Cardano.PlutusList.new();
  fieldsInner.add(Loader.Cardano.PlutusData.new_bytes(fromHex(owner)));
  fieldsInner.add(Loader.Cardano.PlutusData.new_bytes(fromHex(firstSlot)));
  fieldsInner.add(Loader.Cardano.PlutusData.new_bytes(fromHex(lastTransfer)));
  const ownership = Loader.Cardano.PlutusList.new();
  ownership.add(
    Loader.Cardano.PlutusData.new_constr_plutus_data(
      Loader.Cardano.ConstrPlutusData.new(
        Loader.Cardano.Int.new_i32(0),
  ...
  const datum = Loader.Cardano.PlutusData.new_constr_plutus_data(
    Loader.Cardano.ConstrPlutusData.new(
      Loader.Cardano.Int.new_i32(DATUM_TYPE.Ownership),
      ownership
    )
  );
  return datum
...
const TOKENTRACE = ({ tMinted, treasuryValue }) => {
  const fieldsInner = Loader.Cardano.PlutusList.new();
  fieldsInner.add(Loader.Cardano.PlutusData.new_bytes(fromHex(tMinted)));
  fieldsInner.add(Loader.Cardano.PlutusData.new_bytes(fromHex(treasuryValue)));
  const tokenTrace = Loader.Cardano.PlutusList.new();
  tokenTrace.add(
    Loader.Cardano.PlutusData.new_constr_plutus_data(
      Loader.Cardano.ConstrPlutusData.new(
        Loader.Cardano.Int.new_i32(0),
  ...
  const datum = Loader.Cardano.PlutusData.new_constr_plutus_data(
    Loader.Cardano.ConstrPlutusData.new(
      Loader.Cardano.Int.new_i32(DATUM_TYPE.TokenTrace),
      tokenTrace
    )
  );
  return datum
...
class YoctoDao {
  ...
  /**
   * This function returns a list of UTxOs that have a Datum in the
   * metadata slot 405, reside at the script address, and have at least
   * one of the tokens provided as an argument.
   * @returns {Utxo[]}
   */
  async getUtxo(policy, name) {
    const govAsset = policy + fromAscii(name);
  ...
  /**
   * Feels like the point of this is pretty obvious.. Starting point for
   * *any* transaction that we are constructing here.
   *@private
   */
  async initTx() {
  ...
  /**
   * Not sure what this is either, also TODO.
   * @private
   */
